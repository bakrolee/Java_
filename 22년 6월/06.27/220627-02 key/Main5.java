...
public class Main5 extends JFrame {
	...
	public Main5() {
		...
		btnLogin.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (!(members.get(tfMain.getText()) == null)) {
					String pw = members.get(tfMain.getText());
					if (pw.equals(pfMain.getText())) {
		...
		btnCheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// 1. 비밀번호 확인
				else if (!pfPW.getText().equals(pfPW2.getText())) {
					JOptionPane.showMessageDialog(null, "비밀번호가 다릅니다.");
					pfPW.setText("");
					pfPW2.setText("");
				}
				// 2. ID, PW 길이 확인 (4자 이상, 12자 이하)
				else if (!(lengthCk(tfID.getText()) && lengthCk(pfPW.getText()))) {
				...
				// 3. 회원정보 map에 등록
					...
					members.put(tfID.getText(), pfPW.getText());
	...
	// 글자 길이 체크
	public boolean lengthCk(String s) {
